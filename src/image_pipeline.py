...
def process_image(path: Path, options: ProcessingOptions, face_cropper: Optional[FaceCropper]) -> ImageResult:
    ...
    crop_box.size = min(crop_box.size, width, height)
    crop_box.x = clamp(crop_box.x, 0, width - crop_box.size)
    crop_box.y = clamp(crop_box.y, 0, height - crop_box.size)
